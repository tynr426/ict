...
namespace Core.BLL
{
    public class AuthProductBusiness : CoreBusiness
    {
        public AuthProductBusiness(IDapperFactory dapperFactory) : base(dapperFactory)
        {
        ...
        public int Delete(string authProductIds, int connectorId)
        {
            ...
            foreach (string authId in ids)
            {
                ...
                if (id > 0)
                {
                    AuthProductEntity ent = (AuthProductEntity)new AuthProductBusiness(_dapperFactory).GetEntity("Id="+ id+ " and ConnectorId="+ connectorId);
        ...
        public PagingResult GetAuthProductList(PagingQuery pagingQuery,int connectorId,bool auth=true)
        {
            ...
            AuthBrandEntity brands = (AuthBrandEntity)new AuthBrandBusiness(_dapperFactory).GetEntity("ConnectorId=" + connectorId);
            AuthCategoryEntity categorys = (AuthCategoryEntity)new AuthCategoryBusiness(_dapperFactory).GetEntity("ConnectorId=" + connectorId);
            ...
            //(dict["Content"] as JObject)["Data"]
            JObject content = dict["Content"] as JObject;
        ...
        public PagingResult UpConnectorAuthList(PagingQuery pagingQuery, int connectorId) {
            ...
            string str = string.Format("select * from {0} where DownConnectorId={1} and Status=1 and IsDefault=1 limit 1", RelationBusiness._TableName, connectorId);
            ...
            if (productList.Data!=null) {
                ...
                if (dt == null || dt.Rows.Count <= 0)
                {
                    ...
                    foreach (JObject item in (JArray)productList.Data)
                    {
