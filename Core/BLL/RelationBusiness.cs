...
namespace Core.BLL
{
    public class RelationBusiness : CoreBusiness
    {
        public RelationBusiness(IDapperFactory dapperFactory) : base(dapperFactory)
        {
        ...
        public IResultResponse CheckDownConnector(int connectorId,int id,int status)
        {
            try
            {
                ...
                ConnectorEntity downConnector = (ConnectorEntity)new ConnectorBusiness(_dapperFactory).GetEntity("Id=" + entRelation.DownConnectorId);
                if (downConnector == null || Utils.ToInt(downConnector.Id) == 0)
                {
