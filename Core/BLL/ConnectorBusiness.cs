...
namespace Core.BLL
{
    public class ConnectorBusiness : CoreBusiness
    {

        public ConnectorBusiness(IDapperFactory dapperFactory) : base(dapperFactory)
        {
        ...
        public IResultResponse Join(Dictionary<string, object> dic)
        {
            ...
            using (DTService dbHandler = new DTService(_dapperFactory))
            {
                try
                {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public DataTable GetDownList(int id)
        {
            return DbAccess.GetDataTable(string.Format(CommandConstant.GetDownConnectorListById, id));
        }

        #region 库存查询 所有下游合作商列表
        public PagingResult DownConnectorStock(int downConnectorId, PagingQuery pagingQuery)
        {
            PagingResult result = new PagingResult()
            {
            ...
            ConnectorEntity connector = (ConnectorEntity)new ConnectorBusiness(_dapperFactory).GetEntity("Id=" + downConnectorId);
            if (connector == null)
            {
                return result;
            }
            ...
            IResultResponse res = ApiRequest.GetResponse(group.Domain, "mall.management.productpage", dic);
            if (res.Success)
            {

                result = res.Content as PagingResult;
